...
if (isset($_GET['del'])) {
    ...
    if ($target && strpos($target, ROOT_PATH) === 0 && $target !== ROOT_PATH) {
        if (is_dir($target)) { rmdirRecursive($target); $msg = 'Folder deleted.'; }
        else                 { unlink($target);          $msg = 'File deleted.'; }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mkdir'])) {
    ...
    if (@mkdir($newDir)) { $msg = 'Folder created.'; }
    else                 { $msg = 'Error.'; $msgType = 'err'; }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['rename_from'])) {
    ...
    if ($from && strpos($from, ROOT_PATH) === 0) {
        if (rename($from, $to)) { $msg = 'Renamed successfully.'; }
        else                    { $msg = 'Error.'; $msgType = 'err'; }
    }
}
...
if (isset($_GET['view'])) {
    ...
    if ($vFile && is_file($vFile) && strpos($vFile, ROOT_PATH) === 0
...
body { background: var(--bg0); color: var(--txt); font-family: var(--font); font-size: 14px; min-height: 100dvh; }

/* ── Header ── */
.header { background: var(--bg1); border-bottom: 1px solid var(--line); padding: 12px 16px; display: flex; align-items: center; gap: 10px; position: sticky; top: 0; z-index: 50; }
.header-logo { font-size: 18px; font-weight: 700; color: var(--acc); letter-spacing: -0.5px; flex-shrink: 0; }
.header-logo span { color: var(--txt2); font-weight: 400; font-size: 13px; margin-left: 4px; }
.search-wrap { flex: 1; position: relative; }
.search-input { width: 100%; background: var(--bg2); border: 1px solid var(--line); border-radius: 8px; padding: 8px 12px 8px 36px; color: var(--txt); font-size: 13px; outline: none; }
.search-input:focus { border-color: var(--acc); }
.search-ico { position: absolute; left: 10px; top: 50%; transform: translateY(-50%); color: var(--txt3); font-size: 15px; pointer-events: none; }
.btn-logout { background: none; border: 1px solid var(--line); color: var(--txt2); border-radius: 8px; padding: 7px 10px; font-size: 12px; cursor: pointer; white-space: nowrap; }

/* ── Breadcrumb ── */
.breadcrumb { padding: 10px 16px; display: flex; align-items: center; gap: 4px; flex-wrap: wrap; border-bottom: 1px solid var(--line); background: var(--bg1); }
.bc-item { color: var(--txt2); font-size: 12px; text-decoration: none; }
.bc-item:hover { color: var(--acc); }
.bc-sep { color: var(--txt3); font-size: 11px; }
.bc-current { color: var(--txt); font-size: 12px; font-weight: 600; }

/* ── Message ── */
.msg { margin: 12px 16px 0; padding: 10px 14px; border-radius: var(--radius); font-size: 13px; border-left: 3px solid; }
.msg.ok  { background: rgba(62,207,142,.1); border-color: var(--ok); color: var(--ok); }
.msg.err { background: rgba(247,101,101,.1); border-color: var(--err); color: var(--err); }

/* ── Toolbar ── */
.toolbar { padding: 12px 16px; display: flex; gap: 8px; flex-wrap: wrap; border-bottom: 1px solid var(--line); }
.btn { display: inline-flex; align-items: center; gap: 5px; padding: 8px 12px; border-radius: 8px; font-size: 12px; font-weight: 600; cursor: pointer; border: none; text-decoration: none; transition: opacity .15s; }
.btn:active { opacity: .7; }
.btn-primary { background: var(--acc); color: #fff; }
.btn-ghost   { background: var(--bg2); color: var(--txt2); border: 1px solid var(--line); }
.btn-danger  { background: rgba(247,101,101,.15); color: var(--err); border: 1px solid rgba(247,101,101,.2); }

/* ── Upload area ── */
.upload-zone { margin: 0 16px 0; background: var(--bg2); border: 1.5px dashed var(--line); border-radius: var(--radius); padding: 14px; text-align: center; display: none; }
.upload-zone.open { display: block; }
.upload-zone input { display: none; }
.upload-zone label { color: var(--acc); cursor: pointer; font-size: 13px; }
.upload-zone .hint { color: var(--txt3); font-size: 11px; margin-top: 4px; }
.upload-filename { color: var(--txt2); font-size: 12px; margin-top: 6px; }
...
.modal { background: var(--bg2); border-radius: 16px 16px 0 0; width: 100%; max-width: 480px; padding: 20px 16px 32px; }
.modal h3 { font-size: 15px; margin-bottom: 12px; color: var(--txt); }
.modal input[type=text] { width: 100%; background: var(--bg3); border: 1px solid var(--line); border-radius: 8px; padding: 10px 12px; color: var(--txt); font-size: 14px; outline: none; margin-bottom: 10px; }
.modal input:focus { border-color: var(--acc); }
.modal-actions { display: flex; gap: 8px; }
.modal-actions .btn { flex: 1; justify-content: center; }

/* ── File list ── */
.section-title { padding: 14px 16px 6px; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: .8px; color: var(--txt3); }
.file-list { padding: 0 8px 80px; }
.file-item { display: flex; align-items: center; gap: 10px; padding: 10px 10px; border-radius: var(--radius); margin-bottom: 2px; cursor: pointer; user-select: none; position: relative; }
.file-item:hover, .file-item.selected { background: var(--bg2); }
.file-icon { font-size: 22px; flex-shrink: 0; width: 36px; text-align: center; }
.file-meta { flex: 1; min-width: 0; }
.file-name { font-size: 14px; color: var(--txt); font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.file-info { font-size: 11px; color: var(--txt3); margin-top: 2px; }
.file-actions { display: flex; gap: 6px; opacity: 0; transition: opacity .15s; }
.file-item:hover .file-actions { opacity: 1; }
.file-actions a, .file-actions button { background: var(--bg3); border: none; color: var(--txt2); border-radius: 6px; padding: 5px 8px; font-size: 12px; cursor: pointer; text-decoration: none; }
.file-actions .del-btn { color: var(--err); }
.empty { padding: 40px 16px; text-align: center; color: var(--txt3); font-size: 13px; }

/* ── Viewer ── */
.viewer { position: fixed; inset: 0; background: var(--bg0); z-index: 200; display: none; flex-direction: column; }
.viewer.open { display: flex; }
.viewer-header { background: var(--bg1); border-bottom: 1px solid var(--line); padding: 12px 16px; display: flex; align-items: center; gap: 10px; }
.viewer-title { flex: 1; font-size: 14px; font-weight: 600; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.viewer-close { background: none; border: 1px solid var(--line); color: var(--txt2); border-radius: 8px; padding: 6px 12px; font-size: 13px; cursor: pointer; }
.viewer-body { flex: 1; overflow: auto; padding: 16px; }
.viewer-body pre { font-family: 'SF Mono', 'Fira Code', monospace; font-size: 12px; line-height: 1.7; color: var(--txt); white-space: pre-wrap; word-break: break-all; }

/* ── Search results ── */
.search-res { padding: 0 16px; }
.search-res-item { display: flex; align-items: center; gap: 8px; padding: 10px 0; border-bottom: 1px solid var(--line); }
.search-res-item a { color: var(--acc); font-size: 13px; text-decoration: none; }
.search-res-path { color: var(--txt3); font-size: 11px; margin-top: 2px; }

/* ── FAB ── */
.fab { position: fixed; bottom: 24px; right: 16px; background: var(--acc); color: #fff; border: none; border-radius: 50%; width: 52px; height: 52px; font-size: 24px; cursor: pointer; box-shadow: 0 4px 20px rgba(124,106,247,.4); z-index: 40; display: flex; align-items: center; justify-content: center; }

@media (prefers-color-scheme: light) { :root { } } /* force dark */
</style>
</head>
<body>
...
<!-- Header -->
<div class="header">
  <div class="header-logo">X-Plore <span>fm</span></div>
...
<div class="file-item" onclick="location.href='<?= $baseUrl ?>path=<?= urlencode($parentRel) ?>'">
  ...
  <div class="file-meta"><div class="file-name">.. Back</div></div>
</div>
<?php endif ?>
...
async function viewFile(path, name) {
  ...
  } catch(e) { document.getElementById('viewerContent').textContent = 'Error: ' + e.message; }
}
function closeViewer() { document.getElementById('viewer').classList.remove('open'); }

// ── Modals ──
function openMkdir()  { document.getElementById('mkdirModal').classList.add('open'); }
function closeMkdir() { document.getElementById('mkdirModal').classList.remove('open'); }
...
body{background:#0c0c0e;color:#d8d8e8;font-family:'SF Pro Display','Segoe UI',system-ui,sans-serif;min-height:100dvh;display:flex;align-items:center;justify-content:center;padding:20px}
.card{background:#131318;border:1px solid #2a2a38;border-radius:16px;padding:32px 24px;width:100%;max-width:360px}
.logo{font-size:30px;font-weight:800;color:#7c6af7;text-align:center;letter-spacing:-1px}
.sub{text-align:center;color:#555566;font-size:12px;margin:4px 0 28px}
label{display:block;font-size:12px;color:#888899;margin-bottom:6px}
input[type=password]{width:100%;background:#1a1a22;border:1px solid #2a2a38;border-radius:10px;padding:12px 14px;color:#d8d8e8;font-size:15px;outline:none;margin-bottom:16px}
input[type=password]:focus{border-color:#7c6af7}
button{width:100%;background:#7c6af7;color:#fff;border:none;border-radius:10px;padding:13px;font-size:15px;font-weight:700;cursor:pointer}
.err{background:rgba(247,101,101,.1);color:#f76565;border-radius:8px;padding:10px;font-size:13px;text-align:center;margin-bottom:14px;border:1px solid rgba(247,101,101,.2)}
